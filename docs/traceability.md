...
| Cross-platform application | A contract applies on another operating system when nothing in it is platform-restricted, and names the package when something is | `packages/companion/src/portability.ts`, `packages/adapters/src/npm.ts` | `packages/companion/src/portability.test.ts` |
| Autocapture | A recorder starts by itself, announces itself, survives a reboot on all three platforms, and gives up rather than failing forever | `packages/companion/src/daemon.ts`, `packages/companion/src/autostart.ts`, `apps/cli/src/cli.ts` | `packages/companion/src/daemon.test.ts`, `packages/companion/src/autostart.test.ts`, `tests/timeline-e2e.test.ts` |
